function [LDA_Projection,Eigenvectors] = LDAfunc(Data)
...
% NOTE: Newer Matlab version encourages usage of "Matrix Division" instead of multiplication with inv(). This is because inv() function in Matlab is slow, buggy, and unreliable due to historical reasons. In most cases inv() should work just fine. If this can cause confusion, please revert S_B/S_w back to inv(S_w)*S_B.
